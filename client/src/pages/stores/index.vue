...
useHead({ title: () => t('entities.stores') })
...
const sort = computed(() => {
  ...
  return {
    property: property as StoreSort,
    direction: direction as SortDirection,
  } satisfies Sort<StoreSort>
})
...
const { preference: columnVisibility } = useUserPreference<Record<string, boolean>>('stores_column_visibility', {
  modifiedAt: false,
  createdAt: true,
  location: false,
  legalName: false,
...
const { preference: columnOrder } = useUserPreference<ColumnOrderState>('stores_column_order', ['name', 'type', 'createdAt'])
</script>
