...
import type { TagEntity, TagSort } from '@/types/spinestack-tag'
...
const columnHelper = createColumnHelper<TagEntity>()
  ...
  columnHelper.display({
    ...
    cell: ({ row }) => h(
      Button,
      {
        ...
        isRouterLink: true,
        class: 'w-10 h-10',
        to: { name: 'tags-id', params: { id: row.original.id } },
      },
      {
        default: () => [
...
<template>
  <Table
    ...
    :data="tags?.data"
    :columns="columns"
    :page-count="tags?.pagination?.totalPages"
    :items-count="tags?.pagination?.totalElements"
    :loading="isLoading"
  >
    <template #empty>
      <slot name="empty" />
