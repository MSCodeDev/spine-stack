...
import io.github.mscodedev.spinestack.domain.model.ROLE_USER
import io.github.mscodedev.spinestack.domain.model.SpineStackUser
import io.github.mscodedev.spinestack.domain.model.makeUuid
...
class UserControllerTest(
  ...
  @SpykBean
  private lateinit var userLifecycle: SpineStackUserLifecycle
  ...
  fun `it should return bad request when creating a user with invalid email`(email: String) {
    val jsonString = """{"name": "User", "email": "$email", "password": "password"}"""
  ...
  inner class Update {
    ...
    fun `it should update the roles of an existing user without roles`() {
      val user = SpineStackUser("user@example.org", "", false, "User", id = makeUuid())
    ...
    fun `it should update the roles of an existing user with roles`() {
      val user = SpineStackUser("user@example.org", "", true, "User", id = makeUuid())
      userLifecycle.createUser(user)
